...
A linear regression model for mRNA expression $y$ (suppressing the index $j$ for clarity) is defined as
\begin{equation}
...
\begin{equation}
	min\{ || y - X \cdot w ||_2 + \lambda \cdot R(w) \} ,
\end{equation}
...
Many methods use the 1-norm ($R(w) = ||w||_1 = \sum_{k=0}^{K} |w_k|$)
...
\begin{equation}
  y | w, \sigma, X \tilde N(Xw, \sigma^2I),
\end{equation}
