...
templates, but have different biogenesis, where miRNAs are cut from hairpin
strucutres and siRNAs are processed from long double-stranded RNAs (dsRNAs).
...
through Ago4, in what is called the RNA-induced silencing complex (RISC)
RISC is a protein complex containing Dicer, TRBP and the Ago \citep{Gregory2005}.
...
The RISC functions as the effector of miRNA-induced mRNA repression.
Ago acts as the heart of the RISC and the miRNA bound to it serves
to guide the RISC to target messenger RNAs \citep{}. Target
matching is based on sequence complimentarity.
...
\item
  P-bodies (not essential)
\item
  combinatorial effect (many-to-one)
\item
  miRNAs can also induce methylation (Kawasaki2004) and other unusual mechanisms
\end{itemize}
