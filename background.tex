...
or poorly (III) differentiated tumors
with less differentiated tumors having worse prognosis
...
correlated with the OnocotypeDx recurrence score in breast cancer
...
mRNA and protein expression poorly correlated \cite{tahanOliNiitaRefeja}.
mRNA only acts as a proxy. Some evidence that differential expression analysis
still makes sense, better correlation in DEGs \cite{seYksPaperi}.
...
latest miRTarBase) using automated text mining
\citep{Dweep2015}. The only tools that currently use the most recent version of
miRBase (v21) are TargetScan and rna22.
...
\begin{itemize}
  \item First published Bayesian analysis by Thomas Bayes in 1763 \citep{Gelman2013}.
  \item Bayes' rule
