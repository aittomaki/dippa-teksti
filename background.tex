...
\subsubsection{Bayesian approaches}\label{bayesian-approaches}
...
about uknown parameters $\vartheta$ given some observed data $y$. This is
achieved through probability statements.

Bayesian analysis begins by defining a joint probability distribution
$p(y,\vartheta)$, which can be written as a product of two densities
\begin{equation*}
  p(y,\vartheta) = p(\vartheta)*p(y|\vartheta),
\end{equation*}
which are referred to as the \emph{prior distribution} $p(\vartheta)$ and the
data distribution $p(y|\vartheta)$ (or \emph{likelihood}, when taken as a
function of $\vartheta$). By applying the Bayes' rule we obtain the
\emph{posterior distribution} for $\vartheta$ given the known values for the
data $y$:
\begin{equation}
  \label{eq:bayes}
  p(\vartheta|y) = \frac{p(y,\vartheta)}{p(y)} = \frac{p(\vartheta)*p(y|\vartheta)}{p(y)} \propto p(\vartheta)*p(y|\vartheta),
\end{equation}
where $p(y) = \int_{\vartheta} p(\vartheta)*p(y|\vartheta) d\vartheta$ (over
all possible values of $\vartheta$) and the proportionality follows from
noting that $p(y)$ does not depend on $\vartheta$. p(\vartheta)*p(y|\vartheta)
