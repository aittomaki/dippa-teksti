...
Considering the recent large body of research on microRNAs, and their potential
utility as biomarkers and treatment targets, it is not surprising that a
plethora of computational tools have been released to aid in microRNA
research. Knowing microRNA targets is essential to understand their
biological function and a large number of methods have also been developed
for microRNA target prediction, which we will focus
on here.
...
As discussed above, the microRNA seed sequence (nucleotides 2-7) is the primary
determinant in target mRNA recognition by the RISC. Therefore, putative miRNA
targets can be identified by searching for the appropriate
seed sequence within mRNA sequences (primarily in the 3' UTR) \citep{Bartel2009},
found matches are called miRNA target sites or seed matches.
