...
reflected in mRNA and protein abundances. Sequence-based
predictions are most often incorporated as a PRELIMINARY filter step to
limit the potential interactions examined. Examples of methods
...
of expression values of miRNAs $k (k = 1, 2, ..., J)$ for N observations.
...
\paragraph{Simple similarity measures}
The relationship between miRNAs and mRNAs can be easily analyzed using
measures of similarity, such as correlation or mutual information (MI).
The goal is to find miRNA-mRNA pairs whose expression pattern is similar
or correlated across observations. The drawback of MI is that it only indicates similarity of the variables,
but not the direction, or sign, of the relationship.
Pearson correlation
is widely used because of its simplicity and intuitive interpretation.
Pearson correlation between mRNA $j$ and miRNA $k$ is defined as: 
\begin{equation}
	\rho_{jk} = (\mathbf{x_j})_{\mu_k=0|\sigma_k=1}^T \cdot (\mathbf{z_k})_{\mu_k=0|\sigma_k=1}^T,
	\label{eq:correlation}
\end{equation}
where $\mu_i=0|\sigma_i=1$ indicates
normalization to zero mean and standard deviation of one and $\codt$ a vector
dot product.
miRNA-mRNA pairs significantly correlated are classified as targets. Simple
correlation, however, often results in very high numbers of putative targets and
many tools use additional information, such as differential expression
and sequence-based putative targets, to limit results \citep{Muniategui2010}.
The drawback of such measures is that they can only be applied for
pairwise miRNA-mRNA comparisons.

\paragraph{Multiple linear regression}
Multiple linear regression (MLR) can be used to examine the 
relationship between miRNAs and mRNAs. Most tools use expression of miRNAs
to predict mRNA (or protein) expression. In the case of MLR, target
prediction becomes a variable selection problem, where the
goal is to choose miRNAs that are best able to predict mRNA expression.
A linear model for mRNA $j$ can be defined as:
\begin{equation}
	\mathbf{x_j} = \sum_{k=0}^{K} (w_{jk} \cdot \mathbf{z_k} + \epsilon_{jk}) = \mathbf{w_j} \cdot \mathbf{Z} + \mathbf{\epsilon_j},
\end{equation}
where $\mathbf{w_j}$ is the vector of regression coefficients,
$w_0$ is the intercept term, $\epsilon_j$ is the error term (often
assumed to be normally distributed), and
$\mathbf{Z}$ is the matrix of covariates, i.e. the miRNA expression vectors.
The parameter of interest is then $\mathbf{w_j}$, which \textbf{SIGNIFIES}
to the contribution of each miRNA to the response variable (mRNA expression).
Commonly the solution is obtained by least squares, which involves
minimizes the objective function,
\begin{equation}
	min\{ || \mathbf{x_j} - \mathbf{w_j} \cdot \mathbf{Z} ||_2 \} ,
\end{equation}
which is equal to the sum of squares of the residuals, or fitting errors.
The advantage of using regression for target prediction is the possibility
of modelling the effect of several miRNAs simultaneously. This is desirable
as the effect of a single miRNA on mRNA expression can be small, as discussed above.

\paragraph{Regularized regression}
When the number of covariates (here $K$) is larger than the number of
samples, the simple linear model is undetermined and 
regular MLR is not applicable. This is quite often the case with high-throughput biological data analysis, for example microarray expresssion data. There are several solutions to this problem,
one of which is using regularization. In regularized least squares a penalty
term is applied to the coefficients $\mathbf{w_j}$ to force them small.
This approach involves minimizing,
\begin{equation}
	min\{ || \mathbf{x_j} - \mathbf{w_j} \cdot \mathbf{Z} ||^2_2 + \lambda \cdot R(\mathbf{w_j}) \} ,
\end{equation},
where $R(\mathbf{w_j})$ is the penalty function and $\lambda$ is a tuning parameter that
controls the amount of regularization. Commonly used regularizations include
the 1-norm ($R(\mathbf{w_j}) = ||\mathbf{w_j}||_1$) in lasso regression,
the 2-norm ($R(\mathbf{w_j}) = ||\mathbf{w_j}||_2$) in ridge regression
and a combination of these
($R(\mathbf{w_j}) = \lambda_1||\mathbf{w_j}||_1 + \lambda_2||\mathbf{w_j}||_2$) 
in what is called elastic-net regression. Lasso regression in effect
forces the number of non-zero coefficients to be small, where as ridge
regression results in a solution where coefficients are small but
mostly non-zero. While regularization solves the dimensionality problem and aids in interpretability, it has several drawbacks. First, only a limited number of covariates may be included in the model, and
