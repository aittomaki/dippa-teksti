...
\begin{table}
  ...
  \begin{tabular}{ llllp{4cm} }
    ...
    JOKUTOINENRULE & \cite & Rule based &  &  \\
...
\subsubsection{Expression-data-based target prediction}\label{expr-methods}
...
% \begin{equation}
% 	min\{ || \mathbf{x_j} - \mathbf{Z} \cdot \mathbf{w_j} ||_2 \},
% \end{equation}
...
as probability distributions \citep{Gelman2013}. This is advantageous as
probabilities are often easy to comprehend and common-sense interpretations
can be made.
\footnote{This is especially true compared to classical frequentist analysis,
which is conceptualized through infinite repetitions. For example, frequentist
confidence intervals do not indicate that the true value of a parameter is
contained within with a certain probability -- a common misconception -- where
as Bayesian posterior intervals do (subject to model assumptions, of course).}

Bayesian analysis starts with the definition of the joint probability model,
$p(x,\mu)$, where $x$ are the observed data and $\mu$ the unknown
model parameters. By application of the Bayes' rule, we ob
\begin{equation}
	\label{bayes-rule}
\end{equation}
Equation \ref{bayes-rule} demonstrates how the posterior distribution
is a combination of prior knowledge and information contained in the
observed data (with the latter dominating as the number of observations
increases).

Several features make Bayesian modeling very flexible. First, each parameter
may have a different prior distribution. Second, parameters of these prior
distributions can also have priors, which are called hyperpriors. Models
including hyperpriors are referred to as hierarchical models (referring to
several layers of parameterization) and, in principle, any number of parameter
layers can be easily included. Third, additional data can be included sequentially
using the previously obtained posterior as the new prior distribution.

In cases where no certain prior information is available, weakly informative
or uninformative priors can be used. These convey little or no
information on the presumed values a parameter may take, respectively.

\begin{itemize}
  \item The first published Bayesian analysis by Thomas Bayes in 1763 .
  \item Bayes' rule
...
JÄTÄ VAIN TOINEN KAHDESTA EKASTA. LYHENNÄ TÄTÄ MUUTEN?
...
distribution $p(\vartheta|y)$, where possible, or a distribution proportional
to $p(\vartheta|y)$. More commonly Markov chain methods are used. These entail
drawing iterative samples of $\vartheta$ from a distribution that approaches
the true posterior during iteration rounds, where each draw $\vartheta^t$ is
conditional (only) on the previous one $\vartheta^{t-1}$.
...
different starting values $\vartheta^0$ to assure proper coverage of the whole
...
\paragraph{Bayesian regression and variable selection}
...
\subsubsection{Bayesian approaches to target prediction}
