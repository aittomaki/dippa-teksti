...
This Section presents an overview of published methods from both classes.

% For more in-depth reviews, see for example \citep{Yue2009,Muniategui2012}.
...
Sequence-based prediction methods focus on finding miRNA and mRNA pairs that
...
pair-wise prediction of miRNA-mRNA targets is essentially a classification
...
miRNA-mRNA duplex. These rule-based prediction methods are unsupervised, i.e. no training data
...
from previous literature or expression data sets -- is used to train a
classifier for classifying any miRNA-mRNA pair. Most commonly a support vector
machine (SVM) is used as the classifier. The features used for classification
are similar to rule-based tools, namely sequence features, but supervised
learning allows the inclusion of many more features. For example mirTarget
uses a set of 113 features including seed matches, conservation and a range of
different sequence features from different parts of the miRNA sequence
\citep{mirTarget}.
...
of sequence-based methods include: considering only direct regulation in pair-
wise interactions cannot capture indirect regulation and combinatorial
...
suggesting that many results are spurious false positives \citep{Muniategui2012}.
...
\subsubsection{Expression-data-based target prediction}
...
reflected in mRNA and protein abundances. Mathematical models used in such expression
...
\begin{table}
  ...
  See text for details on several methods. Ref. $=$ Reference,
  Method = statistical method used, Use of seq. data $=$ how the method utilizes sequence-based predictions.}
  \label{table:expression-methods}
  \centering
  \begingroup\small
  \begin{tabular}{ lllp{6cm} }
    \\[-1ex] \hline\hline
    Name & Ref. & Method & Use of seq. data & Additional Notes \\
    \hline
    MAGIA & \citep{Sales2010} & Correlation, MI & Filter & Also produces a bipartite network of miRNA-mRNA interactions. \\
    TaLasso & \citep{Muniategui2015} & Lasso regression & Filter &  \\
    Engelmann & \citep{Engelmann} & Least-angle regression & None or filter & \\
    miRNAmRNA & \citep{vanIterson2013} & Global test & Filter & \\
    GenMir++ & \citep{} & Bayesian regression & None or filter & \\
    \hline\hline
    \end{tabular}
    \endgroup
\end{table}
...
between miRNAs and mRNAs. Most approaches use expression profiles of miRNAs to predict mRNA
(or protein) expression. Recently Engelmann and showed that miRNA
expression can indeed be used to predict mRNA expression.
 n the case of regression, target prediction essentially
becomes a variable selection problem, where the goal is to choose a set of miRNAs
that are the best predictors of mRNA expression without overfitting. A
...
There exist many solutions to the data dimensionality problem, one of which
