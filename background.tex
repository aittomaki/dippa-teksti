...
Let us henceforth define $Z_{n \times J} = [z_j] = [z_{ij}]$ as the
matrix of expression values of mRNAs $j (j = 1, 2, ..., J)$ and
$X_{n \times K} = [x_k] = [x_{ik}]$ as the matrix of expression
values of miRNAs $k (k = 1, 2, ..., K)$ for observations $i (i = 1, 2, ...,
...
vector $x_0 = [1, ..., 1]^T$ has been added to account for the intercept term.
...
$k$ is defined as:
\begin{equation}
	\rho_{jk} = (z_j)_{\mu_j=0|\sigma_j=1}^T \cdot (x_k)_{\mu_k=0|\sigma_k=1},
	\label{eq:pearson}
\end{equation}
...
\begin{equation}
  \label{eq:linear-regression}
	z_j = \sum_{k=0}^{K} (w_{jk} \cdot x_k + \epsilon_{jk}) =  X \cdot w_j + \epsilon_j,
\end{equation}
where $w_j = [w_j0, ..., w_jK]$ is the vector of regression coefficients,
$w_j0$ is the intercept term, $\epsilon_j$ is the error term and $X$ is the
matrix of covariates, i.e. the miRNA expression vectors. The parameter of
interest is $w_j$, which determines to the contribution of each miRNA to the
response variable, i.e. mRNA expression. The regression error $\epsilon_j$
...
where a penalty depending on the magnitude of the coefficients $w_j$
is applied to force them small. This entails minimizing the expression
\begin{equation}
	min\{ || x_j - X \cdot w_j ||_2 + \lambda \cdot R(w_j) \} ,
\end{equation}
...
Many methods use the 1-norm ($R(w_j) = ||w_j||_1 = \sum_{k=0}^{K} |w_{jk}|$)
for regularization, which is referred to as lasso regression.
% , where 
% regularizations include the 1-norm ($R(w_j) = ||w_j||_1$) in
% lasso regression, the 2-norm ($R(w_j) = ||w_j||_2$) in ridge
% regression and a combination of these ($R(w_j) =
% \lambda_1||w_j||_1 + \lambda_2||w_j||_2$) in what is called
...
\begin{equation}
  \label{eq:bayes}
  p(\theta|y) = \frac{p(y,\theta)}{p(y)} = \frac{p(\theta) \cdot p(y|\theta)}{p(y)},
\end{equation}
where $p(y) = \int_{\theta} p(\theta) \cdot p(y|\theta) d\theta$.
Noting that $p(y)$ does not depend on $\theta$, we can write
\ref{eq:bayes} as
\begin{equation}
