...
validation was done, and the largest $\lambda$ (i.e. sparsest model) that gave
...
target interactors. For some genes, this criterion was met by
...
\end{equation}
where $\hat{y}_i$ are the predictions made by the model and $\bar{y}$
is the mean of the outcome variable $y$.
$R^2$ indicates the proportion of variance of the outcome variable that is explained
by a statistical model, and can be used as a measure of model fit.
...
do not have a well defined scale. A caveat of
$R^2$ is that in linear regression it often increases monotonically by adding more explanatory
variables. The adjusted $R^2$, defined as $\bar{R}^2 = 1-(1-R^2)\frac{n-1}{n-p}$
...
For measuring simulation chain convergence, the $\hat{R}^2$ measure proposed by Gelman et al
was used.
